...
// Time settings
A3W_startHour = 4;                // In-game hour at mission start (0 to 23) - time is saved and restored between server restarts if A3W_timeSaving = 1
...
//A3W_bleedingTime = 60;           // Time in seconds for which to allow revive after a critical injury (minimum 10 seconds) - no effect in ACE Medical
...
A3W_remoteBombStoreRadius = 0;    // Prevent players from placing remote explosives within this distance from any store (0 = disabled)
...
A3W_vehicleQuantity = 350;         // Approximate number of land vehicles to be spawned in towns
...
A3W_territoryPayroll = 0;          // Periodically reward sides and indie groups based on how many territories they own (0 = no, 1 = yes)
