...
_setupObjects =
{
	...
	[
		["UH1C", "UH1C_HH"],
		["uh1h_Shark", "UH1C_XM16AS"],
		["UH1Y_Venom_50Cal", "UH1N_Bulletstorm"]
	];

	if (missionDifficultyHard) then
	{
		(_heliChoices select 0) set [0, "AH1W"];
		(_heliChoices select 1) set [0, "UH1C_XM21AS"];
		(_heliChoices select 2) set [0, "Cobra2"];
	};
	...
	_createVehicle =
	{
		...
		switch (true) do
		{
			case (_type isKindOf "UH1H_FFV" || _type isKindOf "UH1Y_Venom_50Cal" || _type isKindOf "uh1h_Shark" || _type isKindOf "UH1C_HH" || _type isKindOf "UH1C_XM16AS" || _type isKindOf "UH1C_XM21AS" || _type isKindOf "UH1N_Bulletstorm" || _type isKindOf "UH1C"):
			{
			...
			case (_type isKindOf "Cobra2" || _type isKindOf "AH1W"):
			{
...
_successExec =
{
	...
	[_box3, "mission_USSnipers"] call fn_refillbox;
